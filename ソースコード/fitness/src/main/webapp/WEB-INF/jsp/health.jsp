...
<body>
	<div class="container mt-5">
		<h1 class="text-center text-warning py-2">健康状態の記録一覧</h1>
				<p class="sub-text text-center">最大90件表示します。BMI25以上は<span class="text-danger">赤字</span>で表示します。</p>
				...
				<table class="table-striped table-bordered">
					...
					<c:forEach var="healthRecordModel" items="${healthRecordModels}">
						<tr class="scope-row">
							...
							<td class="text col-3<c:if test="${healthRecordModel.bmi>25}"> text-danger</c:if>">${healthRecordModel.bmi}</td>
							<td class="col-1"><form action="./HealthUpdate" method="get">
									<input type="hidden" name="id"
										value="<c:out value="${healthRecordModel.id}" />">
									<button type="submit" class="btn btn-warning text-white">編集</button>
								</form></td>
							<td class="col-1"><form action="./HealthDelete" method="get">
									<input type="hidden" name="id" value="${healthRecordModel.id}">
		...
		<div class="row mb-2">
			<div class="table-responsive">
				<table class="table-striped table-bordered">
					...
					<tr class="scope-row">
						<td class="text col-4">18.50未満</td>
